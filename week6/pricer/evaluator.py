...
class Tester:
    ...
    def chart(self, title):
        ...
        for tr in fig.data:
            scatter = cast(go.Scatter, tr)
            mask = df["color"] == scatter.name
            scatter.customdata = df.loc[mask, ["hover"]].to_numpy()
            scatter.hovertemplate = "%{customdata[0]}<extra></extra>"
            scatter.update(marker={"size": 6})
